...
**In-session step-by-step hint icon**: 💡 floating button, established in `column_addition` (`.col-hint-toggle` in `column_addition.css`, rendered in `index.jsx`) — 42px circle, solid blue `#2563eb`, white 💡 glyph (plain emoji, no SVG), `box-shadow: 0 4px 12px rgba(37,99,235,.32)`, pinned `position: absolute; top/right: calc(12px + var(--app-safe-top/right, 0px))`. Reserve this specifically for "show me the next step while I'm mid-task" (`ColumnHints.jsx`'s coach-mark pattern) — not a generic help/info affordance (that's the separate `ParamsScreen` "i" info-tooltip, or a one-off onboarding modal). Currently only `column_addition` has this; when a new topic needs the same kind of hint, copy this exact spec into its own CSS file (same "duplicated, not shared" reasoning as above) rather than inventing a new icon or color for the same concept.
