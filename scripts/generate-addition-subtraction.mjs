...
const manifest = {
  ...
  modes: [
    ...
    },
    {
      ...
      evaluation: "auto",
      defaultCardId: "operation_plus",
      ui: { title: "8. Листок", instruction: "Перепиши примеры в тетрадь" },
      params: {
        maxNumber: { type: "enum", values: [5, 10, 20], labels: { ru: { "5": "до 5", "10": "до 10", "20": "до 20" } }, default: 10, label: { ru: "Максимальное число" } },
        changeMax: { type: "enum", values: [1, 3, 5, 10, 99], labels: { ru: { "1": "1", "3": "3", "5": "5", "10": "10", "99": "любое" } }, default: 3, label: { ru: "Максимальное изменение" } },
        includeZero: { type: "boolean", default: false, label: { ru: "Включить ноль" } },
      ...
      methodology: {
        text: "Восьмой режим: ребёнок видит листок с готовыми примерами, переписывает их в тетрадь и вычисляет ответы самостоятельно. Примеры состоят из двух действий (A ± B ± C = ?). Переносит навык из экранной среды в письменную работу.",
        tips: [
          "Используйте после уверенного освоения цепочек на экране.",
          "Начинайте с одной группы — скажите «перепиши группу 1».",
          "Разрешайте использовать счётную палку или пальцы при вычислении.",
          "Проверяйте тетрадь совместно: называйте пример вслух и сравнивайте ответы.",
        ],
        duration: "10–15 мин",
      },
    },
    {
      ...
      evaluation: "auto",
      defaultCardId: "operation_plus",
      ui: { title: "9. Слушай и посчитай", instruction: "Слушай пример и вводи ответ", icon: "media/icons/operations_audio.svg" },
      params: {
        maxNumber: {
          type: "enum",
          values: [20, 50, 100],
        ...
        mixedMagnitude: {
          type: "boolean",
          default: true,
          label: { ru: "Разноразрядные числа" },
          hint: { ru: "Например, «5 + 23» — однозначное и двузначное число вместе. Выключите, чтобы оба числа были одного разряда." },
        },
      },
      methodology: {
        ...
        tips: [
          ...
          "Тап по кругляшку-диктору — ребёнок сам решает, когда переслушать пример.",
          "Выключите «Разноразрядные числа», если ребёнок путает 5+23 с 5+2 — так оба числа в примере будут одного разряда (оба однозначные или оба двузначные).",
        ],
        duration: "4–6 мин",
      },
    },
...
const outFile = `public/decks/addition_subtraction_v${manifest.meta.version}.zip`;
