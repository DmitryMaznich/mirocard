import paramiko, io
from cryptography.hazmat.primitives.serialization import (
    load_ssh_private_key, Encoding, PrivateFormat, NoEncryption
)

KEY = b"""-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----"""


def connect():
    pk = load_ssh_private_key(KEY, password=b"=Dmaz241078")
    pem = pk.private_bytes(Encoding.PEM, PrivateFormat.TraditionalOpenSSL, NoEncryption())
    pkey = paramiko.RSAKey.from_private_key(io.StringIO(pem.decode()))
    c = paramiko.SSHClient()
    c.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    c.connect("lcp-81.controlpanel.si", port=22, username="kaplie98", pkey=pkey, timeout=15)
    return c


def run(c, cmd):
    _, out, err = c.exec_command(cmd)
    out.channel.recv_exit_status()
    return out.read().decode(), err.read().decode()


c = connect()

# Test what curl returns from inside the hosting server
print("=== curl mirocard.kaplieva.help/manifest.json (from inside hosting) ===")
out, err = run(c, "curl -sI https://mirocard.kaplieva.help/manifest.json 2>&1")
print(out)

print("\n=== What IP does mirocard.kaplieva.help resolve to? ===")
out, _ = run(c, "dig +short mirocard.kaplieva.help 2>/dev/null || nslookup mirocard.kaplieva.help 2>/dev/null | tail -5")
print(out)

print("\n=== Apache access log for mirocard ===")
out, _ = run(c, "ls /home/kaplie98/logs/ 2>/dev/null | grep mirocard")
print(out or "(no log files)")

out, _ = run(c, "tail -5 /home/kaplie98/logs/mirocard.kaplieva.help-access_log 2>/dev/null")
print(out or "(no access log)")

print("\n=== Does .htaccess work? (test with wget) ===")
out, err = run(c, "wget -S -O /dev/null 'https://mirocard.kaplieva.help/manifest.json' 2>&1 | head -20")
print(out or err)

print("\n=== Parent .htaccess check ===")
out, _ = run(c, "ls -la /home/kaplie98/public_html/.htaccess 2>/dev/null")
print(out or "(no .htaccess in public_html)")
out, _ = run(c, "head -20 /home/kaplie98/public_html/.htaccess 2>/dev/null")
print(out)

print("\n=== PHP curl to test proxy from inside hosting ===")
php = """<?php
$ch = curl_init('https://mazpc.taile45e98.ts.net/manifest.json');
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_TIMEOUT, 10);
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
$r = curl_exec($ch);
$info = curl_getinfo($ch);
echo "Status: " . $info['http_code'] . "\\n";
echo "Size: " . strlen($r) . "\\n";
echo "Content: " . substr($r, 0, 200) . "\\n";
"""
out, err = run(c, f"php -r '{php}' 2>&1")
print(out or err)

c.close()
