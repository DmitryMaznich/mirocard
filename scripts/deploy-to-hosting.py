"""Deprecated direct static deploy to shared hosting for mirocard.kaplieva.help."""
raise SystemExit(
    "Deprecated deploy path. Use `npm run deploy:prod` so production is served "
    "from the single Windows/Caddy runtime."
)
import paramiko, io, os, stat
from pathlib import Path
from cryptography.hazmat.primitives.serialization import (
    load_ssh_private_key, Encoding, PrivateFormat, NoEncryption
)

KEY = b"""-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----"""

DIST = Path(r"C:\Users\dmazn\Projects\Mirocard2\dist")
DECKS = Path(r"C:\Users\dmazn\Projects\Mirocard2\public\decks")
REMOTE_ROOT = "/home/kaplie98/public_html/mirocard"


def connect():
    pk = load_ssh_private_key(KEY, password=b"=Dmaz241078")
    pem = pk.private_bytes(Encoding.PEM, PrivateFormat.TraditionalOpenSSL, NoEncryption())
    pkey = paramiko.RSAKey.from_private_key(io.StringIO(pem.decode()))
    c = paramiko.SSHClient()
    c.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    c.connect("lcp-81.controlpanel.si", port=22, username="kaplie98", pkey=pkey, timeout=15)
    return c


def run(c, cmd):
    _, out, err = c.exec_command(cmd)
    out.channel.recv_exit_status()
    return out.read().decode(), err.read().decode()


def sftp_put(sftp, local, remote):
    sftp.put(str(local), remote)
    print(f"  ↑ {local.name} → {remote}")


def sftp_mkdir(sftp, path):
    try:
        sftp.stat(path)
    except FileNotFoundError:
        sftp.mkdir(path)


def main():
    print("Connecting...")
    c = connect()
    sftp = c.open_sftp()
    print("Connected.\n")

    # 1. Upload static files from dist/
    static_files = ["index.html", "manifest.json", "sw.js",
                    "icon-192.png", "icon-512.png", "apple-touch-icon.png",
                    "favicon.svg"]
    print("=== Uploading static files ===")
    for name in static_files:
        local = DIST / name
        if local.exists():
            sftp_put(sftp, local, f"{REMOTE_ROOT}/{name}")

    # 2. Upload decks
    print("\n=== Uploading decks ===")
    remote_decks = f"{REMOTE_ROOT}/decks"
    sftp_mkdir(sftp, remote_decks)
    for f in DECKS.iterdir():
        if f.is_file():
            sftp_put(sftp, f, f"{remote_decks}/{f.name}")

    # 3. Write SPA .htaccess (no proxy, just SPA routing)
    print("\n=== Writing .htaccess ===")
    htaccess = (
        "Options -Indexes\n"
        "RewriteEngine On\n"
        "RewriteBase /\n"
        "\n"
        "# Serve existing files directly\n"
        "RewriteCond %{REQUEST_FILENAME} -f\n"
        "RewriteRule ^ - [L]\n"
        "\n"
        "# SPA fallback\n"
        "RewriteRule ^ index.html [L]\n"
        "\n"
        "# No-cache for app shell\n"
        '<FilesMatch "^(index\\.html|sw\\.js|manifest\\.json)$">\n'
        '    Header set Cache-Control "no-store, no-cache, must-revalidate"\n'
        '</FilesMatch>\n'
    )
    with sftp.open(f"{REMOTE_ROOT}/.htaccess", "w") as f:
        f.write(htaccess)
    print("  ✓ .htaccess written")

    # 4. Verify
    print("\n=== Remote files ===")
    out, _ = run(c, f"ls -lh {REMOTE_ROOT}/ && echo '---' && ls -lh {REMOTE_ROOT}/decks/")
    print(out)

    sftp.close()
    c.close()
    print("\nDone! Check https://mirocard.kaplieva.help/")


if __name__ == "__main__":
    main()
