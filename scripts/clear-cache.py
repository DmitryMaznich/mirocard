import paramiko, io
from cryptography.hazmat.primitives.serialization import (
    load_ssh_private_key, Encoding, PrivateFormat, NoEncryption
)

KEY = b"""-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----"""

FUNNEL = "https://laptop-353ltno0.taile45e98.ts.net"

def connect():
    pk = load_ssh_private_key(KEY, password=b"=Dmaz241078")
    pem = pk.private_bytes(Encoding.PEM, PrivateFormat.TraditionalOpenSSL, NoEncryption())
    pkey = paramiko.RSAKey.from_private_key(io.StringIO(pem.decode()))
    c = paramiko.SSHClient()
    c.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    c.connect("lcp-81.controlpanel.si", port=22, username="kaplie98", pkey=pkey, timeout=15)
    return c

def run(c, cmd):
    _, out, err = c.exec_command(cmd)
    out.channel.recv_exit_status()
    return out.read().decode(), err.read().decode()

c = connect()
sftp = c.open_sftp()

# LiteSpeed cache purge via .htaccess
htaccess = (
    "Options -Indexes\n"
    "\n"
    "# Disable LiteSpeed cache for this virtualhost\n"
    "<IfModule LiteSpeed>\n"
    "    CacheEngine off\n"
    "</IfModule>\n"
    "\n"
    "RewriteEngine On\n"
    "\n"
    "<IfModule mod_proxy.c>\n"
    "    ProxyRequests Off\n"
    "    ProxyPreserveHost Off\n"
    f"    ProxyPass / {FUNNEL}/\n"
    f"    ProxyPassReverse / {FUNNEL}/\n"
    "</IfModule>\n"
    "\n"
    "<IfModule !mod_proxy.c>\n"
    f"    RewriteRule ^(.*)$ {FUNNEL}/$1 [R=302,L]\n"
    "</IfModule>\n"
)

with sftp.open("/home/kaplie98/public_html/mirocard/.htaccess", "w") as f:
    f.write(htaccess)

# Also try the LiteSpeed cache purge via a special PHP file
purge_php = '<?php header("X-LiteSpeed-Purge: *"); echo "purged"; ?>'
with sftp.open("/home/kaplie98/public_html/mirocard/purge.php", "w") as f:
    f.write(purge_php)

print("Updated .htaccess with CacheEngine off")
print("Created purge.php")

# Trigger the purge
out, err = run(c, "curl -sk https://mirocard.kaplieva.help/purge.php 2>&1")
print(f"Purge response: {out or err}")

# Wait a moment and test
import time
time.sleep(3)

out, err = run(c, "curl -sI https://mirocard.kaplieva.help/manifest.json 2>&1")
print(f"\nAfter purge — manifest.json headers:\n{out or err}")

sftp.close()
c.close()
