/**
 * Removes the category param from season_form_pick — the mode always
 * shows all seasons, no filter needed. Title and icon are kept.
 *
 * Usage: node scripts/patch-wf-v1.0.49-sfp-remove-params.mjs
 */
import fs from "fs";
...
const SRC_ZIP   = "word_formation_soup_v1.0.48.zip";
const NEW_VER   = "1.0.49";
...
const updatedModes = srcTopic.modes.map(mode => {
  ...
  const { params: _removed, ...rest } = mode;
  return rest;
});
...
console.log("params:", sfp.params ?? "(none)");
