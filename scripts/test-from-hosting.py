import paramiko, io
from cryptography.hazmat.primitives.serialization import (
    load_ssh_private_key, Encoding, PrivateFormat, NoEncryption
)

KEY = b"""-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----"""


def connect():
    pk = load_ssh_private_key(KEY, password=b"=Dmaz241078")
    pem = pk.private_bytes(Encoding.PEM, PrivateFormat.TraditionalOpenSSL, NoEncryption())
    pkey = paramiko.RSAKey.from_private_key(io.StringIO(pem.decode()))
    c = paramiko.SSHClient()
    c.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    c.connect("lcp-81.controlpanel.si", port=22, username="kaplie98", pkey=pkey, timeout=15)
    return c


def run(c, cmd):
    _, out, err = c.exec_command(cmd)
    out.channel.recv_exit_status()
    return out.read().decode(), err.read().decode()


c = connect()

# Test curl to Tailscale URL from inside hosting
print("=== curl from hosting to Tailscale (headers) ===")
out, err = run(c, "curl -sI https://mazpc.taile45e98.ts.net/ 2>&1")
print(out or err)

print("\n=== curl from hosting to Tailscale (content first 200 chars) ===")
out, err = run(c, "curl -sk https://mazpc.taile45e98.ts.net/ 2>&1 | head -c 300")
print(out or err)

# Check what headers .htaccess proxy sends
print("\n=== curl mirocard.kaplieva.help with verbose ===")
out, err = run(c, "curl -sIL https://mirocard.kaplieva.help/ 2>&1 | head -30")
print(out or err)

# Check .htaccess more carefully
print("\n=== Current .htaccess ===")
out, _ = run(c, "cat /home/kaplie98/public_html/mirocard/.htaccess")
print(out)

# Check if there's an error log showing proxy errors
print("\n=== Apache/LiteSpeed error log ===")
out, _ = run(c, "tail -20 /home/kaplie98/logs/mirocard.kaplieva.help-ssl_log 2>/dev/null")
print(out or "(no ssl log)")

c.close()
