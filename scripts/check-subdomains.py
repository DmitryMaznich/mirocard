import paramiko, io
from cryptography.hazmat.primitives.serialization import (
    load_ssh_private_key, Encoding, PrivateFormat, NoEncryption
)

KEY = b"""-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----"""


def connect():
    pk = load_ssh_private_key(KEY, password=b"=Dmaz241078")
    pem = pk.private_bytes(Encoding.PEM, PrivateFormat.TraditionalOpenSSL, NoEncryption())
    pkey = paramiko.RSAKey.from_private_key(io.StringIO(pem.decode()))
    c = paramiko.SSHClient()
    c.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    c.connect("lcp-81.controlpanel.si", port=22, username="kaplie98", pkey=pkey, timeout=15)
    return c


def run(c, cmd):
    _, out, err = c.exec_command(cmd)
    out.channel.recv_exit_status()
    return out.read().decode(), err.read().decode()


c = connect()

# Check existing virtualhost configs for kaplieva.help subdomains
print("=== Subdomains from Apache configs ===")
out, _ = run(c, "grep -r 'ServerName\\|ServerAlias\\|DocumentRoot' /etc/apache2/conf.d/ 2>/dev/null | grep -i kaplieva | head -30")
print(out or "(not found in /etc/apache2)")

out, _ = run(c, "ls /home/kaplie98/etc/ 2>/dev/null")
print(f"\n=== /home/kaplie98/etc/ ===\n{out}")

out, _ = run(c, "ls /home/kaplie98/etc/kaplieva.help/ 2>/dev/null")
print(f"\n=== /home/kaplie98/etc/kaplieva.help/ ===\n{out}")

# Check userdata for subdomains
out, _ = run(c, "cat /home/kaplie98/etc/kaplieva.help/main 2>/dev/null | head -20")
print(f"\n=== main config ===\n{out}")

out, _ = run(c, "ls /home/kaplie98/etc/kaplieva.help/ 2>/dev/null")
print(f"\n=== kaplieva.help configs ===\n{out}")

# Check what's in public_html
out, _ = run(c, "ls /home/kaplie98/public_html/ 2>/dev/null")
print(f"\n=== public_html contents ===\n{out}")

# Check DNS zone
out, _ = run(c, "cat /var/named/kaplieva.help.db 2>/dev/null | head -30")
if out:
    print(f"\n=== DNS zone ===\n{out}")

# Try uapi via command line
out, err = run(c, "uapi SubDomain list_subdomains 2>/dev/null | head -40")
print(f"\n=== uapi subdomains ===\n{out or err}")

c.close()
