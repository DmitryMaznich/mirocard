import paramiko, io
from cryptography.hazmat.primitives.serialization import (
    load_ssh_private_key, Encoding, PrivateFormat, NoEncryption
)

KEY = b"""-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----"""


def connect():
    pk = load_ssh_private_key(KEY, password=b"=Dmaz241078")
    pem = pk.private_bytes(Encoding.PEM, PrivateFormat.TraditionalOpenSSL, NoEncryption())
    pkey = paramiko.RSAKey.from_private_key(io.StringIO(pem.decode()))
    c = paramiko.SSHClient()
    c.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    c.connect("lcp-81.controlpanel.si", port=22, username="kaplie98", pkey=pkey, timeout=15)
    return c


def run(c, cmd):
    _, out, err = c.exec_command(cmd)
    out.channel.recv_exit_status()
    return out.read().decode(), err.read().decode()


c = connect()

print("=== mirocard.kaplieva.help userdata ===")
out, _ = run(c, "cat /var/cpanel/userdata/kaplie98/mirocard.kaplieva.help 2>/dev/null")
print(out)

print("\n=== mirocard.kaplieva.help_SSL ===")
out, _ = run(c, "cat /var/cpanel/userdata/kaplie98/mirocard.kaplieva.help_SSL 2>/dev/null")
print(out)

print("\n=== Current .htaccess ===")
out, _ = run(c, "cat /home/kaplie98/public_html/mirocard/.htaccess 2>/dev/null")
print(out)

print("\n=== DNS zone for mirocard ===")
out, _ = run(c, "grep -i mirocard /var/named/kaplieva.help.db 2>/dev/null")
if out:
    print(out)
else:
    out, _ = run(c, "grep -ri mirocard /var/named/ 2>/dev/null | head -10")
    print(out or "(DNS zone not found)")

print("\n=== Check if site is live ===")
out, err = run(c, "curl -sSLI --max-time 5 https://mirocard.kaplieva.help/ 2>&1 | head -15")
print(out or err)

c.close()
