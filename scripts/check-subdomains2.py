import paramiko, io
from cryptography.hazmat.primitives.serialization import (
    load_ssh_private_key, Encoding, PrivateFormat, NoEncryption
)

KEY = b"""-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----"""


def connect():
    pk = load_ssh_private_key(KEY, password=b"=Dmaz241078")
    pem = pk.private_bytes(Encoding.PEM, PrivateFormat.TraditionalOpenSSL, NoEncryption())
    pkey = paramiko.RSAKey.from_private_key(io.StringIO(pem.decode()))
    c = paramiko.SSHClient()
    c.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    c.connect("lcp-81.controlpanel.si", port=22, username="kaplie98", pkey=pkey, timeout=15)
    return c


def run(c, cmd):
    _, out, err = c.exec_command(cmd)
    out.channel.recv_exit_status()
    return out.read().decode(), err.read().decode()


c = connect()

# Try correct uapi function names
print("=== uapi SubDomain listsubdomains ===")
out, err = run(c, "uapi SubDomain listsubdomains 2>&1 | head -30")
print(out or err)

# Check the v1 directory for reference
print("\n=== v1 in public_html ===")
out, _ = run(c, "ls /home/kaplie98/public_html/v1/ 2>/dev/null | head -10")
print(out)

# Check if mirocard subdomin already exists in userdata
print("\n=== userdata subdomains ===")
out, _ = run(c, "ls /var/cpanel/userdata/kaplie98/ 2>/dev/null | grep -i mirocard")
print(out or "(none found)")

out, _ = run(c, "ls /var/cpanel/userdata/kaplie98/ 2>/dev/null")
print(f"All userdata:\n{out}")

# Add subdomain via cpapi2
print("\n=== Adding subdomain via cpapi2 ===")
out, err = run(c, "cpapi2 --user=kaplie98 SubDomain addsubdomain rootdomain=kaplieva.help domain=mirocard dir=/public_html/mirocard disallowdot=0 2>&1")
print(out or err)

c.close()
