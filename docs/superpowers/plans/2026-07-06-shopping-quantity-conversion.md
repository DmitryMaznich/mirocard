...
Открой приложение (см. вывод `npm run dev` для адреса, обычно `http://localhost:8080`). Зайди в Планировщик → Меню, выбери 2–3 рецепта, где есть картошка/лук/гречка/творог и хотя бы один рецепт с маслом/солью/какао (например `chicken_potato_oven` + `cocoa`). Перейди на экран «Покупки».

Проверь:
- картошка/лук показывают округлённую величину в кг/г, а не «шт»;
- творог показывает «N пачка»/аналог, а не граммы;
- масло, соль, какао — показаны чекбоксом без числа рядом.

- [ ] **Step 3: Проверить пересинхронизацию через Меню**

На экране Меню отметь любой ингредиент-приправу (например, «соль») как «Купить», даже если рецепт её не требует явно (или использует уже отмеченный пантри-статус). Вернись на экран «Покупки» — товар должен появиться отмеченным, без числа рядом.

- [ ] **Step 4: Остановить dev-сервер**

Останови процесс (Ctrl+C в терминале, где запущен `npm run dev`).
