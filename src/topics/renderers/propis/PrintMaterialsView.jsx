...
// Copied from src/topics/renderers/print_materials/index.jsx (2026-09-15), not imported
// cross-topic: propis owns its own "print_materials" mode now (see topic.json's `categories`/
// `items`, migrated in verbatim from that topic) and must stay buildable on its own even if
// the standalone print_materials topic/renderer is later deleted -- it's hidden, not routed
// to, as of this migration (see docs/propis.md). `topicRecord.meta.id` below resolves to
// "propis" here, which is exactly right: the PDFs/thumbnails this reads via topics.getFile
// live in propis's OWN deck zip now, not print_materials's.
export default function PrintMaterialsView({ topicRecord }) {
