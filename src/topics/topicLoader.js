...
const DEFAULT_MODE_METHODOLOGY = {
  ...
  addition_subtraction: {
    ...
    operation_name_action: {
      summary: "Рука приносит или забирает предмет — ребёнок называет действие.",
      text: "Мост от «что стало» (больше/меньше) к «что сделали» (прибавили/убрали): на экране есть тот, кто действует, — рука. Слова те же, что в следующих режимах: «прибавь — прибавили», «убери — убрали».",
      settings: [
        "Начинайте с диапазона до 3, кнопками, без шага «Сколько?».",
        "«Скажи вслух» — экспрессивная речь: ребёнок сам произносит глагол, взрослый отмечает ✓ или ↻. Вводите после уверенного выбора кнопками.",
        "Шаг «Сколько?» готовит к записи «+2»: рука ходит по одному предмету, чтобы можно было посчитать.",
      ],
      goal: "Ребёнок называет действие — прибавили или убрали — опираясь на действие руки, а не только на итог.",
    },
    operation_do_action: {
  ...
  addition_subtraction: [
    ...
    },
    {
      ...
      params: {
        maxNumber: { type: "enum", values: [3, 5], labels: { ru: { "3": "до 3", "5": "до 5" } }, default: 3, label: { ru: "Максимальное число" } },
        shapeMode: { type: "enum", values: ["circle", "blocks"], labels: { ru: { circle: "Только круги", blocks: "Фигуры сериями" } }, default: "circle", label: { ru: "Материал" } },
        answerMode: { type: "enum", values: ["buttons", "voice"], labels: { ru: { buttons: "Кнопки", voice: "Скажи вслух" } }, default: "buttons", label: { ru: "Ответ" }, hint: { ru: "«Скажи вслух»: ребёнок называет действие сам, взрослый отмечает ✓ или ↻" } },
        countStep: { type: "boolean", default: false, label: { ru: "Спросить «Сколько?»" }, hint: { ru: "Рука делает 1–3 хода, после действия ребёнок выбирает, сколько" } },
      },
    },
    {
  ...
  column_addition: [
    ...
    },
    {
      ...
      params: {
        ...
        digits: {
          type: "enum",
          values: [2, 3, "2+1", "round10"],
          labels: { ru: { "2": "2-зн.", "3": "3-зн.", "2+1": "2-зн. + 1-зн.", round10: "Круглые дес." } },
          default: 2,
          label: { ru: "Разрядность" },
          section: "Что решаем",
          info: {
            ru: {
              text: "Сколько цифр в числах примера — двузначные (например, 34+18), трёхзначные (246+137), «2-зн. + 1-зн.» — двузначное и однозначное число вместе (например, 34+7), где у нижнего числа просто нет разряда десятков, — или «Круглые дес.» — оба числа кратны 10 (10+20, 40+60), сумма никогда не больше 100.",
              tip: "«2-зн. + 1-зн.» — отдельная трудность: ребёнок должен понять, что однозначное число встаёт под разряд единиц, а не пишет 0 в десятках. «Круглые дес.» — тоже отдельный шаг: считать десятками как единицами (4+6=10 → 40+60=100), без внимания к нулям в конце; сумма ограничена сотней, чтобы не уходить в трёхзначные раньше времени. Переходите на 3-значные только когда 2-значные примеры с переносом/займом решаются уверенно.",
            },
          },
        },
        showHelper: {
    ...
    },
    {
      ...
      params: {
        ...
        digits: {
          ...
          info: {
            ru: {
              text: "Сколько цифр в числах примера — двузначные, трёхзначные, «2-зн. + 1-зн.» (двузначное и однозначное число вместе, например 34+7) или «Круглые дес.» (оба числа кратны 10, например 30+40).",
