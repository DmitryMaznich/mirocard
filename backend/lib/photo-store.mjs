...
/** Marks links the account stopped referencing (past the grace period) as released. */
export function releaseUnreferencedPhotos(db, accountId, { at = now(), graceHours = PHOTO_UNREFERENCED_GRACE_HOURS } = {}) {
  const cutoff = new Date(new Date(at).getTime() - graceHours * 3600_000).toISOString();
  const active = db.prepare(
    "SELECT hash, created_at FROM photo_owners WHERE account_id = ? AND released_at IS NULL",
  ).all(accountId);
  if (!active.length) return 0;
  const refs = referencedPhotoHashes(db, accountId);
  const release = db.prepare("UPDATE photo_owners SET released_at = ? WHERE hash = ? AND account_id = ?");
  let released = 0;
  for (const row of active) {
    if (!refs.has(row.hash) && row.created_at < cutoff) {
      release.run(at, row.hash, accountId);
      released += 1;
    }
  }
  return released;
...
/** Re-activates released links for hashes the account references again (never creates links). */
export function reactivateReferencedPhotos(db, accountId, hashes) {
  const stmt = db.prepare(
    "UPDATE photo_owners SET released_at = NULL WHERE hash = ? AND account_id = ? AND released_at IS NOT NULL",
  );
  for (const hash of hashes) stmt.run(hash, accountId);
}
...
export function getPhotoUsage(db, accountId) {
  const row = db.prepare(`
    ...
    WHERE o.account_id = ? AND o.released_at IS NULL
  `).get(accountId);
  return { count: row.count, bytes: row.bytes };
}

function assertWithinQuota(db, accountId, addBytes, limits) {
  releaseUnreferencedPhotos(db, accountId, { graceHours: limits.graceHours });
 ...
 * Re-storing a photo the account already actively owns costs no quota.
 * `enforceQuota: false` is only for migrating data that already exists.
 */
export function storeNormalizedPhoto(db, accountId, normalized, { enforceQuota = true, quota = DEFAULT_QUOTA } = {}) {
  ...
  const ts = now();
  const link = db.prepare("SELECT released_at FROM photo_owners WHERE hash = ? AND account_id = ?").get(hash, accountId);
  const alreadyActive = link && link.released_at === null;
  if (enforceQuota && !alreadyActive) {
  ...
  if (!link) {
    ...
    // Re-activated: restart the grace clock so it isn't released right away.
    db.prepare("UPDATE photo_owners SET released_at = NULL, created_at = ? WHERE hash = ? AND account_id = ?").run(ts, hash, accountId);
  }
...
export async function resolveSyncOperationPhotos(db, accountId, operations, options = {}) {
  ...
  for (const op of operations) {
    ...
    try {
      ...
      // kv.upsert for instruction steps -- makes that link count again.
      reactivateReferencedPhotos(db, accountId, extractPhotoRefs(op.data));
      accepted.push(op);
    } catch (err) {
